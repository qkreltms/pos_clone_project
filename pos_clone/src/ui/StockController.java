...
public class StockController implements Initializable{
	...
	@FXML ListView<Menu> lvStockList;
	...
	private void setChoiceBoxAndInitList(DataProvider shop) {
		...
		.addListener(new ChangeListener<Number>() {
			...
			public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
				lvStockList = setAndGetList(shopList.get((Integer)newValue));
			}
		});
	}
	...
	private ListView<Menu> setAndGetList(Shop selectedItem) {
		if (selectedItem != null) { 
			...
			lvStockList.setItems(data);
			lvStockList.setCellFactory(new Callback<ListView<Menu>, ListCell<Menu>>() {
				@Override 
